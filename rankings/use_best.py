...
def best_model_ranking(
    ...
    while dataset.has_unseen:
        ...
        if (dataset.n_seen // tuning_interval) <= last_tune and last_ranker is not None:
            ...
            dataset.register_predictions(scores=y_pred, model=last_ranker.name)
        else:
            ...
            for ranker in it_rankers(models):
                ...
                logger.info(
                    f'Initializing and training ranker {ranker.name} '
                ...
                logger.debug(f'Testing ranker {ranker.name} for dataset {dataset.KEY} on {1 - train_proportion:.0%} test set')
                ...
                logger.debug(f'Recall is {rec:.2%}, precision is {prec:.2%} (F1={f1:.2%})for ranker {ranker.name} on dataset {dataset.KEY} ')
            ...
            dataset.register_predictions(scores=y_pred, model=best_ranker.name)
