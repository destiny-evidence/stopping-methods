...
CONFIDENCE_TARGETS = [0.8, 0.9, 0.95, 0.99]
INCLUSION_THRESHOLDS = [0.25, 0.5, 0.75, 0.9]
...
class Method(ABC, Generic[T_scores, T_labels, T_bounds, T_sampling]):
    ...
    def compute(
        ...
        **kwargs: _MethodParams,
    ) -> _LogEntry:
        raise NotImplementedError()

    @classmethod
    def retrospective(
        cls, n_total: int, labels: Labels, scores: T_scores, is_prioritised: T_sampling, bounds: T_bounds, batch_size: int = 100, **kwargs: _MethodParams
    ) -> Generator[_LogEntry, None, None]:
