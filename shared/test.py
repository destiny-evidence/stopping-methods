...
def test_method(
    Method: Type[Method], paramset: _MethodParams, dataset_i: int = 0, batch_size: int = 25, loglevel: int | str = logging.DEBUG
) -> tuple[RankedDataset, list[_LogEntry]]:
    ...
    for batch_i, (_, bounds, (labels, scores, is_prioritised)) in enumerate(dataset.cum_batches(batch_size=batch_size)):
        ...
        with elapsed_timer(logger, f'Batch {batch_i} took {Method.KEY}'):
            results.append(
                base_entry
                | Method.compute(
                    n_total=dataset.n_total, labels=labels, scores=scores, full_labels=dataset.labels, bounds=bounds, is_prioritised=is_prioritised, **paramset
                )
            )
...
def plots(dataset: RankedDataset, results: list[_LogEntry], params: _MethodParams) -> tuple[plt.Figure, plt.Axes]:
    ...
    ax.plot([0, len(curve_rand), n_docs], [0, curve_rand.max(), (curve_rand.max() / len(curve_rand)) * n_docs], ls='--', lw=0.5, label='Random inclusion rate')

    # plot trajectory for perfect ranking
    ax.plot(
        [len(curve_rand), len(curve_rand) + curve.max() - curve_rand.max()], [curve_rand.max(), curve.max()], ls='--', lw=0.5, label='Perfect prioritisation'
    )
    # area random init
    ...
    offset = 0
    for res in results:
        ax.fill_between(np.arange(offset, res['n_seen']), 0, 5, facecolor='green' if res['safe_to_stop'] else 'red', alpha=0.2)
        offset = res['n_seen']
