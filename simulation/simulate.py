    ...
    for file_i, ranking_info_fp in enumerate(info_files):
        ...
        for batch_i, (_, bounds, (labels, scores, is_prioritised)) in enumerate(dataset.cum_batches(batch_size=batch_size)):
            ...
            base_entry = {
                ...
                **{f'ranker-{k}': v for k, v in dataset.info.items() if k.startswith('model-')},
                **{f'sampling-{k}': v for k, v in dataset.info.items() if k.startswith('batch-')},
            }
                        ...
                        rows.append(
                            {
                                **base_entry,
                                ...
                                'method-hash': (method.KEY + '-' + sha1(json.dumps(paramset, sort_keys=True).encode('utf-8')).hexdigest()),
                                **{f'method-{k}': v for k, v in stop_result.items()},
                            }
                        )
