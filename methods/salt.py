            ...
            for tr in RECALL_TARGETS:
                for alpha in [0.1, 0.3, 0.5, 0.8, 1.0]:
                    for adjusted in [True, False]:
                        ...
                        yield MethodParams(alpha=alpha, nstd=2, use_adjusted=adjusted, use_margin=False, confidence_level=cl, recall_target=tr)
                        #  - SALt has nstd = 0, use_margin = False;
                        yield MethodParams(alpha=alpha, nstd=0, use_adjusted=adjusted, use_margin=False, confidence_level=cl, recall_target=tr)
                        #  - SAL^r_t has nstd = 0, use_margin = True;
                        yield MethodParams(alpha=alpha, nstd=0, use_adjusted=adjusted, use_margin=True, confidence_level=cl, recall_target=tr)
    ...
    def compute(
        ...
        if use_adjusted:
            _, scores_adjusted = sld(
                ...
                trust=1
                - cosine(
                    scores[idxs_batch, 1],  # FIXME `scores` should actually be scores before retraining
                    scores[idxs_batch, 1],
                ),
            )
            sc = np.copy(scores[:, 1])
            sc[idxs_unseen] = scores_adjusted[:, 1]
            error = np.sum(np.abs(labels[idxs_batch].mean() - sc[idxs_batch].mean())) / (2 * (1 - labels[idxs_batch].mean()))

            if error > alpha:
                sc = np.copy(scores[:, 1])
        else:
            _, scores_adjusted = sld(scores[idxs_unseen], scores[idxs_unseen])  # FIXME `scores` should actually be scores before retraining
            sc = np.copy(scores[:, 1])
            sc[idxs_unseen] = scores_adjusted[:, 1]
        ...
        return LogEntry(
            ...
            recall_target=recall_target,
            est_recall=recall,
            margin_recall=margin_target_recall,
        )


def sld(
    scores_seen: BinaryScores,
    scores_unseen: BinaryScores,
    epsilon=1e-6,
    max_steps: int = 1000,
    adjust: bool = False,
    trust: float = 0.5,
...
def quant_stop(probs: Scores, idxs_seen: Indices, idxs_unseen: Indices, idxs_batch: Indices, nstd: int) -> float:
    ...
    est_var = (known_sum**2 / (known_sum + te_probs.sum()) ** 4 * all_var) + (1 / (known_sum + te_probs.sum()) ** 2 * (all_var - unknown_var))
...
if __name__ == '__main__':
    ...
    #  - SALtQuantCI has nstd = 2;
    params = MethodParams(alpha=1.0, nstd=2, use_adjusted=True, use_margin=False, confidence_level=0.9, recall_target=0.9)
    dataset, results = test_method(SALT, params, 2)
    fig, ax = plots(dataset, results, params)
    fig.show()

    # - SALt has nstd = 0, use_margin = False;
    params = MethodParams(alpha=1.0, nstd=0, use_adjusted=True, use_margin=False, confidence_level=0.9, recall_target=0.9)
    dataset, results = test_method(SALT, params, 2)
    fig, ax = plots(dataset, results, params)
    fig.show()

    #  - SAL^r_t has nstd = 0, use_margin = True;
    params = MethodParams(alpha=1.0, nstd=0, use_adjusted=True, use_margin=True, confidence_level=0.9, recall_target=0.9)
    dataset, results = test_method(SALT, params, 2)
    fig, ax = plots(dataset, results, params)
    fig.show()
