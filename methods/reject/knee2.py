...
def detect_knee(data: np.ndarray, window_size: int = 1, s: float = 10) -> list[int]:
    ...
    for i in range(data_size):
        ...
        if sum_index_weight > 0:  # weighted mean
            smoothed_data.append((sum_x_weight / sum_index_weight, sum_y_weight / sum_index_weight))
        else:
...
class Knee(AbstractMethod):
            ...
            for stopping_beta in [100, 1000]:
                for rho_target in [3, 6, 10]:
                    for n_windows in [10, 50, 100]:
                        yield KneeParamSet(mode=mode, stopping_beta=stopping_beta, rho_target=rho_target, n_windows=n_windows)
    ...
    def compute(
        ...
        if len(list_of_labels) < stopping_beta:  # beta stopping condition (not enough screened, i.e., < beta)
            return KneeLogEntry(safe_to_stop=False, mode=mode, stopping_beta=stopping_beta, rho_target=rho_target, rho=0.0, n_windows=n_windows)
        ...
        data = np.array([np.arange(len(labels)) / self.dataset.n_total, labels.cumsum() / labels.sum()]).T
        data = data[::batch_size]
        actual_n_windows = len(data)
        if actual_n_windows < 2:
            return KneeLogEntry(safe_to_stop=False, mode=mode, stopping_beta=stopping_beta, rho_target=rho_target, rho=0.0, n_windows=actual_n_windows)

        knee_indices = detect_knee(data)
        if not knee_indices:
            return KneeLogEntry(
                safe_to_stop=False, mode=mode, stopping_beta=stopping_beta, rho_target=rho_target, rho=0.0, n_windows=actual_n_windows, knees=[]
            )

        # rho calculation from last knee
        last_knee = knee_indices[-1]
        r1 = (
            data[last_knee][1] * labels.sum()
        )  # de-normalise, atm inclusion curve’s x-axis is “proportion of collection” and y-axis is “recall”, so must undo that scaling before computing ρ.
